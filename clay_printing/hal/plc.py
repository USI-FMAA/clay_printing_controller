...
class PLC(BaseModel):
  ...
  lock_dict: Any = Field(default_factory=Lock, description="Lock for dictionary operations")
  lock_ads: Any = Field(default_factory=Lock, description="Lock for ADS communication")

  def __init__(self, **data):
    super().__init__(**data)
    self.connection = pyads.Connection(self.netid, pyads.PORT_TC3PLC1, self.ip)

  def connect(self, report = True) -> bool:
    """Connect to the PLC."""
    ...
    try:
      ...
      if report:
        logger.info(f"PLC Connection: {self.connection.is_open}")
      return True
  ...
  def read_variables(self, variable: str, report=True) -> None:
  ...
  def write_variables(self, variable: str, value: Any) -> None:
    ...
    Raises:
        AdsConnectionError:
    """
    if not self.connect(report=False):
      raise AdsConnectionError("Could not read variable from PLC, PLC connection failed.")
    try:
      self.connection.write_by_name(variable, value)
      logger.success(f"writing {variable}: {value} to PLC")
      return None
    except KeyError as e:
      logger.error(f"Error: {e}")
      return None
